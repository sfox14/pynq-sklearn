...
void LinearRegression(fifoA_t &xbuf, coef_t amem[n_outputs][n_components], coef_t bmem[n_outputs],
...
#pragma HLS UNROLL
		...
		// macc
macc:	for (int i=0; i<n_components; i++) {
#pragma HLS PIPELINE
...
#pragma HLS PIPELINE
				...
				xdat[j] = xdin;
				data_t xval_uint = xdat[j];
				coef_t aval_uint = amem[j][i];
				op_t op1, op2;
